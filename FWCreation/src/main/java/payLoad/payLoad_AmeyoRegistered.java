...
public class payLoad_AmeyoRegistered{
	
	public static Logger log = LogManager.getLogger(payLoad_AmeyoRegistered.class.getName());
	...
	public static String AccountCreationResponse_UATFC() {
		...
		StudentCreation = given().header("Authorization", val + Utils.Access_TokenUATFC())
				...
				.post("https://byjusprod--byjusuatfc.my.salesforce.com/services/data/v51.0/composite").then().log().all()
				.extract().response().asString();
        log.info("The response is: "+StudentCreation);
		return StudentCreation;
