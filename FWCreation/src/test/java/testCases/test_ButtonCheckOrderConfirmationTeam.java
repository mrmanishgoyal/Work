...
public class test_ButtonCheckOrderConfirmationTeam extends base {

	public WebDriver driver;
	//public String CurrURL;
	public static Logger log = LogManager.getLogger(test_ButtonCheckOrderConfirmationTeam.class.getName());
	...
	public void TestButtonOrderConfirmationTeam() throws Exception {
		...
		if(CurrURL.contains("--byjusuatfc")) {
			...
			al2 = excelData.getData("OrderConfirmationTeam User UATFC", "Login", "Type");
			log.info("Logging in as Admin to UATFC Env then switching user to OCT");
		...
		al = excelData.getData("TC1", "LSPDCase", "Tcid");
		al2 = excelData.getData("OrderConfirmationTeam User UAT", "Login", "Type");
		
		log.info("Logging in as Admin to UAT then switching user to OCT");
		lo.LoginAsAdmin_UAT();
		//lo.SwitchUser(al2.get(1));
		//log.info("Submitting the Account creation payload");
		...
		ac.CheckCaseListViewButtons(EnvUAT,"Case_OCT");
		}
			...
			cases.CaseNavMenuClick();
			cases.RecentlyViewedlist();
			ac.CheckCaseListViewButtons(EnvUATFC,"Case_OCT");
		}
		else {
			lo.SwitchUsernProfile_Prod("Order Confirmation Team");
			ac.closeTabWindows();
			ac.Notification();
			cases.NavMenuClick();
			cases.CaseNavMenuClick();
			cases.RecentlyViewedlist();
			ac.CheckCaseListViewButtons(EnvProd,"Case_OCT");
		}
		...
		if(CurrURL.contains("--byjusuat")&&!CurrURL.contains("--byjusuatfc")) {
			ccr.CheckCreatedCaseButtonsQAandSM(EnvUAT,"Case_OCT");
		}
		else if(CurrURL.contains("--byjusuatfc")){
			ccr.CheckCreatedCaseButtonsQAandSM(EnvUATFC,"Case_OCT");
		}
		else {
			ccr.CheckCreatedCaseButtonsQAandSM(EnvProd,"Case_OCT");
		}
