...
public class test_ButtonCheckPE extends base {

	public WebDriver driver;
	//public String CurrURL;
	public static Logger log = LogManager.getLogger(test_ButtonCheckPE.class.getName());
	...
	public void TestButtonPE() throws Exception {
		...
		Thread.sleep(5000);
		ac.CheckButtonsQAandSM(EnvUAT,"Account_PE");
		ac.CheckRelatedList(EnvUAT,"Account_PE");
		//ac.EnableButtons();
		//ac.CheckRelatedListButtonVals(EnvUAT, "Account_PE");
		}
			...
			driver.get(AccountURL);
			Thread.sleep(5000);
			ac.CheckButtonsQAandSM(EnvUATFC,"Account_PE");
			ac.CheckRelatedList(EnvUATFC,"Account_PE");
		}
		else {
			...
			driver.get(AccountURL);
			Thread.sleep(5000);
			ac.CheckButtonsQAandSM(EnvProd,"Account_PE");
			ac.CheckRelatedList(EnvProd,"Account_PE");
		}
		//ac.Scrollhome();
		
		StudentSubOrderPO Ssub=new StudentSubOrderPO(driver);
		ac.ClickStudentProg2();
		ac.ClickSSOinStuProg();
		...
		if(CurrURL.contains("--byjusuat")&&!CurrURL.contains("--byjusuatfc")) {
			ibdt.CheckTaskButtonsQAandSM(EnvUAT,"Task_PE");
			}
			else if(CurrURL.contains("--byjusuatfc")){
				ibdt.CheckTaskButtonsQAandSM(EnvUATFC,"Task_PE");
			}
			else {
				ibdt.CheckTaskButtonsQAandSM(EnvProd,"Task_PE");
			}
		...
		if(CurrURL.contains("--byjusuat")&&!CurrURL.contains("--byjusuatfc")) {
		ac.CheckCaseListViewButtons(EnvUAT,"Case_PE");
		}
		else if(CurrURL.contains("--byjusuatfc")){
			ac.CheckCaseListViewButtons(EnvUATFC,"Case_PE");
		}
		else {
			ac.CheckCaseListViewButtons(EnvProd,"Case_PE");
		}
