...
public class test_ButtonCheckRetention extends base {

	public WebDriver driver;
	//public String CurrURL;
	public static Logger log = LogManager.getLogger(test_ButtonCheckRetention.class.getName());
	...
	public void TestButtonRetention() throws Exception {
		...
		if(CurrURL.contains("--byjusuatfc")) {
			...
			al2 = excelData.getData("Retention User UATFC", "Login", "Type");
			log.info("Logging in as Admin to UATFC Env then switching user to Retention");
		...
		log.info("Logging in as Admin to UAT then switching user to Retention");
		lo.LoginAsAdmin_UAT();
		//lo.SwitchUser(al2.get(1));
		log.info("Submitting the Account creation payload");
		...
		Thread.sleep(5000);
		ac.CheckButtonsQAandSM(EnvUAT,"Account_Retention");
		ac.CheckRelatedList(EnvUAT,"Account_Retention");
		}
			...
			driver.get(AccountURL);
			Thread.sleep(5000);
			ac.CheckButtonsQAandSM(EnvUATFC,"Account_Retention");
			ac.CheckRelatedList(EnvUATFC,"Account_Retention");
		}
		else {
			...
			driver.get(AccountURL);
			Thread.sleep(5000);
			ac.CheckButtonsQAandSM(EnvProd,"Account_Retention");
			ac.CheckRelatedList(EnvProd,"Account_Retention");
		}
		...
		if(CurrURL.contains("--byjusuat")&&!CurrURL.contains("--byjusuatfc")) {
			ccr.CheckCreatedCaseButtonsQAandSM(EnvUAT,"Case_Retention");
		}
		else if(CurrURL.contains("--byjusuatfc")){
			ccr.CheckCreatedCaseButtonsQAandSM(EnvUATFC,"Case_Retention");
		}
		else {
			ccr.CheckCreatedCaseButtonsQAandSM(EnvProd,"Case_Retention");
		}
		...
		if(CurrURL.contains("--byjusuat")&&!CurrURL.contains("--byjusuatfc")) {
		ac.CheckCaseListViewButtons(EnvUAT,"Case_Retention");
		}
		else if(CurrURL.contains("--byjusuatfc")){
			ac.CheckCaseListViewButtons(EnvUATFC,"Case_Retention");
		}
		else {
			ac.CheckCaseListViewButtons(EnvProd,"Case_Retention");
		}
