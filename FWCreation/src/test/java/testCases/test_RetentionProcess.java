...
import payLoad.payLoad_RetentionProcess;
import resources.ExcelData;
import resources.base;



public class test_RetentionProcess extends base {

	public WebDriver driver;
	//public String CurrURL;
	public static Logger log = LogManager.getLogger(test_RetentionProcess.class.getName());
	...
	public void TestRetentionProcess() throws Exception {
		...
		if(CurrURL.contains("--byjusuatfc")) {
			...
			Accountid=payLoad_RetentionProcess.AccountidCreationResponse_UATFC();
		...
		lo.LoginAsAdmin_UAT();
		log.info("Submitting the Account creation payload");
		Accountid=payLoad_RetentionProcess.AccountidCreationResponse_UAT();
		log.info("Launching the newly created Account id "+Accountid);
			...
			lo.LoginAsAdmin_QA();
			log.info("Submitting the Account creation payload");
			Accountid=payLoad_RetentionProcess.AccountidCreationResponse_QA();
			log.info("Launching the newly created Account id "+Accountid);
		}
		else {
			al = excelData.getData("TC1", "RefundProcess", "Tcid");
			...
			log.info("Submitting the Account creation payload");
			Accountid=payLoad_RetentionProcess.AccountidCreationResponse_Prod();
			log.info("Launching the newly created Account id "+Accountid);
		}
		...
		//cases.CaseOptnSel();
		//Assert.assertTrue(false);
		ca1.ClickAccOwnerNam();
		ca1.CloseSubTabs();
		
		
		log.info("Creating another Inbound Task for duplicate Refund Request Scenario");
		if(CurrURL.contains("--byjusuat")&&!CurrURL.contains("--byjusuatfc")){
		
		ca1.ClickOpenActiDD2();
		}
		else {
		...
		ibdt.SelectPSTO();
		Thread.sleep(1000);
		Assert.assertTrue(ibdt.CheckErrorMess());
