...
public class test_ButtonCheckLogistics extends base {

	public WebDriver driver;
	//public String CurrURL;
	public static Logger log = LogManager.getLogger(test_ButtonCheckLogistics.class.getName());
	...
	public void TestButtonLogistics() throws Exception {
		...
		if(CurrURL.contains("--byjusuatfc")) {
			...
			al2 = excelData.getData("Logistics User UATFC", "Login", "Type");
			log.info("Logging in as Admin to UATFC Env then switching user to Logistics");
		...
		al = excelData.getData("TC1", "LSPDCase", "Tcid");
		al2 = excelData.getData("Logistics User UAT", "Login", "Type");
		
		log.info("Logging in as Admin to UAT then switching user to Logistics");
		lo.LoginAsAdmin_UAT();
		//lo.SwitchUser(al2.get(1));
		//log.info("Submitting the Account creation payload");
		...
		ac.CheckCaseListViewButtons(EnvUAT,"Case_Logistic");
		}
			...
			cases.CaseNavMenuClick();
			cases.RecentlyViewedlist();
			ac.CheckCaseListViewButtons(EnvUATFC,"Case_Logistic");
		}
		else {
			lo.SwitchUsernProfile_Prod("Logistics Team");
			ac.closeTabWindows();
			ac.Notification();
			cases.NavMenuClick();
			cases.CaseNavMenuClick();
			cases.RecentlyViewedlist();
			ac.CheckCaseListViewButtons(EnvProd,"Case_Logistic");
		}
		...
		if(CurrURL.contains("--byjusuat")&&!CurrURL.contains("--byjusuatfc")) {
			ccr.CheckCreatedCaseButtonsQAandSM(EnvUAT,"Case_Logistic");
		}
		else if(CurrURL.contains("--byjusuatfc")){
			ccr.CheckCreatedCaseButtonsQAandSM(EnvUATFC,"Case_Logistic");
		}
		else {
			ccr.CheckCreatedCaseButtonsQAandSM(EnvProd,"Case_Logistic");
		}
